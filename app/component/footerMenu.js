      ...
      {menuItems.map((item) => (
        <Link href={item.path} key={item.name} className='relative flex-1 flex flex-col items-center justify-center text-center'>
          {item.alert && item.alertCount > 0 && (
            <span className="absolute left-1/2 bottom-1/2 inline-flex items-center justify-center min-w-4.5 h-4.5 text-xs font-bold leading-none text-red-100 bg-red-600 rounded-full">
              {item.alertCount}
            </span>
          )}
