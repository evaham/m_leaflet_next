...
export default function FooterMenu() {
  ...
  const menuItems = [
    {
      ...
      activeicon: (
        <svg xmlns="http://www.w3.org/2000/svg" className='w-7 h-7 fill-blue-500' height="24px" viewBox="0 -960 960 960" width="24px"><path d="M160-120v-480l320-240 320 240v480H560v-280H400v280H160Z" /></svg>
      ),
      alert: false,
      alertCount: 0,
    },
    ...
    },
    {
      ...
      activeicon: (
        <svg xmlns="http://www.w3.org/2000/svg" className='w-7 h-7 fill-blue-500' height="24px" viewBox="0 -960 960 960" width="24px"><path d="M280-80q-33 0-56.5-23.5T200-160q0-33 23.5-56.5T280-240q33 0 56.5 23.5T360-160q0 33-23.5 56.5T280-80Zm400 0q-33 0-56.5-23.5T600-160q0-33 23.5-56.5T680-240q33 0 56.5 23.5T760-160q0 33-23.5 56.5T680-80ZM208-800h590q23 0 35 20.5t1 41.5L692-482q-11 20-29.5 31T622-440H324l-44 80h480v80H280q-45 0-68-39.5t-2-78.5l54-98-144-304H40v-80h130l38 80Z" /></svg>
      ),
      alert: true,
      alertCount: 2,
    },
  ...
  return (
    <div className='fixed right-0 bottom-0 left-0'>
      <nav className='flex h-20 gap-auto bg-white border-t border-gray-200'>
        {menuItems.map((item) => (
          <Link href={item.path} key={item.name} className="relative flex-1 flex flex-col items-center justify-center text-center">
            {item.alert && item.alertCount > 0 && (
              <span className="absolute left-1/2 top-1/2 -mt-8 ml-2 flex items-center justify-center min-w-4.5 h-4.5 text-xs font-bold leading-none text-red-100 bg-rose-500 rounded-full">
                {item.alertCount}
              </span>
            )}
            {pathname === item.path ? item.activeicon : item.icon}
            <div className={`text-xs ${pathname === item.path ? 'text-blue-500 font-bold' : 'text-slate-500'}`}>{item.name}</div>
          </Link>
        ))}
      </nav>
    </div>
